...
class RAGSystem:
    def __init__(self, reranker_handle):
        ...
        logger.info("Initializing local AsyncLLMEngine model=%s", MODEL_NAME)
        ...
        logger.info("External LLM client initialized base_url=%s", self.external_llm_raw_base)

        try:
            logger.info(
                "Connecting to Weaviate http=%s:%s grpc=%s:%s collection=%s",
                WEAVIATE_HTTP_ADDR,
                WEAVIATE_HTTP_PORT,
                WEAVIATE_GRPC_ADDR,
                WEAVIATE_GRPC_PORT,
        ...
        if not self.weaviate_connection.is_ready():
            ...
            raise RuntimeError("Weaviate is not ready, aborting RAGSystem initialization")
        ...
        logger.info("Local AsyncLLMEngine initialized model=%s", MODEL_NAME)
    ...
    async def make_context_prediction(self, req: InputRagQuestion):
        ...
        collection = self.product_collections.get(req.product_name, self.nova_collection)
...
rag_reader_app = RAGSystem.bind(reranker_app)
